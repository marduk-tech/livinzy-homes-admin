...
export function LivindexHospitalList() {
  const { isMobile } = useDevice();
  const { data, isLoading, isError } = useFetchLivindexPlaces({
    type: "hospital",
  });

  const deletePlaceMutation = useDeletePlaceMutation({ type: "hospital" });
      ...
      render: (id: string, record) => {
        return (
          <Flex gap={isMobile ? 5 : 15} justify="end">
            <EditLivIndexPlace selectedPlace={record} type="hospital" />
  ...
  return (
    <>
      ...
      >
        ...
        <Col>
          <EditLivIndexPlace type="hospital" />
        </Col>
      </Row>
